...
async function generateSampleData() {
  ...
  try {
    ...
    await sequelize.query('DELETE FROM departments WHERE id > 1', { transaction: t });
    await sequelize.query('ALTER TABLE departments AUTO_INCREMENT = 2', { transaction: t });
    await sequelize.query('ALTER TABLE employees AUTO_INCREMENT = 1', { transaction: t });
    console.log('  已清理\n');
    ...
    const getDeptId = async (code) => {
      const [rows] = await sequelize.query('SELECT id FROM departments WHERE code = ?', { replacements: [code], transaction: t });
    ...
    await sequelize.query(
      `INSERT INTO departments (id, name, code, level, parent_id, sort_order, description, is_active, created_at, updated_at)
       VALUES (1, ?, ?, 1, NULL, 1, ?, 1, NOW(), NOW())
       ON DUPLICATE KEY UPDATE name=VALUES(name), code=VALUES(code), level=1, sort_order=1, description=VALUES(description), is_active=1, updated_at=NOW()`,
      { replacements: ['信阳移动公司工会', 'XY001', '总公司工会'], transaction: t }
    );
    console.log('  L1: 信阳移动公司工会 (id=1, 确保存在)');
    ...
    for (const d of l2Depts) {
      await sequelize.query(
        ...
        { replacements: [d.name, d.code, d.sort, d.desc], transaction: t }
      );
    }
    console.log(`  L2: ${l2Depts.length} 个部门 (parent_id=1)`);
    ...
    for (const d of l3Depts) {
      await sequelize.query(
        ...
        { replacements: [d.name, d.code, d.parent, d.sort, d.desc], transaction: t }
      );
    }
    ...
    await sequelize.query(
      ...
      { replacements: ['已撤销部门', 'XY099', '已撤销的测试部门'], transaction: t }
    );
    ...
    for (const emp of employees) {
      ...
      await sequelize.query(
        `INSERT INTO employees (name, gender, birthday, phone, department, department_id, department_code, level, position, is_active, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, NOW(), NOW())`,
        { replacements: [emp.name, emp.gender, emp.birthday, emp.phone, emp.dept, deptId, emp.deptCode, emp.level, emp.position], transaction: t }
      );
    }
